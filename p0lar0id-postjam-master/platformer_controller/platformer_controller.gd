...
export var max_jump_height = 180 setget set_max_jump_height
...
#shooting
onready var throw_pos = $Position2D
onready var polaroidPath = preload("res://polaroid.tscn")
...
func _physics_process(delta):
	...
	if Input.is_action_just_released(input_jump):
		holding_jump = false
	#Shooting Here
	if Input.is_action_just_pressed("shoot") and inventory.can_shoot == true and inventory.item_in_inv > 0:
		double_click = false
		shoot()
	...
	get_node("CameraSprite").position = get_local_mouse_position().normalized()*50
	get_node("CameraSprite").rotation = get_local_mouse_position().angle()
	
	
	
	checkBoxCollision(vel)
...
func shoot():
	var polaroid = polaroidPath.instance()
	get_parent().add_child(polaroid)
	polaroid.position = $Node2D/Position2D.global_position
	polaroid.velocity= get_global_mouse_position() - polaroid.position
	polaroid.item = inventory.item_in_inv
	inventory.item_in_inv = 0
	coyote_timer.stop()
...
func checkBoxCollision(vel : Vector2) -> void:
	#https://www.youtube.com/watch?v=l2wF2gj_hlA&ab_channel=GDQuest
	if vel.y != 0:
		return
	var vel2 = vel
	vel2.y = 0
